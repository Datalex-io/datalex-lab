...
from WrapperFunction.azure_vault import AzureKeyVault
from WrapperFunction.databricks import DatabricksDeltaSharingClient
...
async def docs():
  """
  Custom route to serve the Swagger UI documentation.

  This endpoint is used to display the Swagger UI for the FastAPI application, but it is 
  not included in the OpenAPI schema (`include_in_schema=False`). This ensures that the 
  documentation is accessible via `/docs` but is not exposed in the OpenAPI specification 
  itself.

  In the case of Azure Function Authorization (using function keys), the `/docs` 
  endpoint would require an `x-functions-key` header to be accessed. This custom route 
  allows us to serve Swagger UI while keeping the access restricted and secure, in line 
  with Azure's Function Auth.

  :return: An HTML response that renders the Swagger UI for the FastAPI app.
  """
  ...
  try:
    ...
    try:
      ...
      if type(ex).__name__ == "ResourceNotFoundError":
        ...
        table_name = json.loads(AzureKeyVault().get_secret(DELTASHARING_TABLES_NAME).value)['platform_aggregated_usage']
        ...
        share_response = delta_sharing.create_share(f"{client_id}_platform_aggregated_usage", table_name, partitions)
        ...
        update_grant_response = delta_sharing.update_share_permissions(f"{client_id}_platform_aggregated_usage", f"{client_id}")
...
def delete_share(client_id: str):
  ...
  try:
    ...
    share_response = delta_sharing.delete_share(f"{client_id}_platform_aggregated_usage")
